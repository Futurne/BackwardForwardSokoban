...
class MacroSokobanEnv(sokoban_env.SokobanEnv):
    ...
    def reachable_states(self):
        ...
        for box in boxes:
            ...
            for n_board, n_player in macro_moves(board, player, box, forward=True):
...
class BackwardSokobanEnv(sokoban_env.SokobanEnv):
    """Backward Sokoban with macro-moves.

    The backward environment allows only pulls.

    The environment is the same, but now a step
    in the environment is a *backward* macro-move.
    A macro-move is a serie of moves where
    only one box is pulled.
    ...
    def reachable_states(self):
        ...
        for box in boxes:
            # Generate all macro moves for this box
            for n_board, n_player in macro_moves(board, player, box, forward=False):
                n_board[tuple(n_player)] = TYPE_LOOKUP['player']  # Place the player to create a valid room_state
                states.append(n_board)  # Add the macro-move
    ...
    def _check_if_done(self) -> bool:
        ...
        """
        return self._check_if_all_boxes_not_on_target() or self._check_if_maxsteps()
...
if __name__ == '__main__':
    ...
    env = BackwardSokobanEnv((6, 6), num_boxes=2)
    raw = env.reset(render_mode='raw')
    board, player = build_board_from_raw(raw)
    print_board(board, player)
